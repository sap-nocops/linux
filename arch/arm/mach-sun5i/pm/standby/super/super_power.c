...
__s32 mem_power_init(__u32 wakeup_src)
{
	...
	if(wakeup_src & AXP_WAKEUP_USB){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN1, &reg_val)){
				return -1;
			}
			reg_val |= (0x3 << 2);
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_IRQEN1, &reg_val)){
				return -1;
			}

			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_GPIO1CFG, &reg_val)){
				return -1;
			}
			reg_val = 0x82;
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_GPIO1CFG, &reg_val)){
				return -1;
			}
	...
	if(wakeup_src & AXP_WAKEUP_GPIO0_FALLING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}

			/* enable gpio0 edgg irq*/
			reg_val |= (0x1 << 0);
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_GPIO0CFG, &reg_val)){
				return -1;
			}
			/* enable gpio0 as input*/
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio0 falling irq enable*/
			reg_val |= (0x1 << 6);
			
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_GPIO0CFG, &reg_val)){
				return -1;
			}
	}
	if(wakeup_src & AXP_WAKEUP_GPIO0_RAISING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio0 raising irq enable*/
			reg_val |= (0x1 << 7);
	...
	if(wakeup_src & AXP_WAKEUP_GPIO1_FALLING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}

			/* enable gpio1 edgg irq*/
			reg_val |= (0x1 << 1);
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			/* enable gpio1 as input*/
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio1 falling irq enable*/
			reg_val |= (0x1 << 6);
	...
	if(wakeup_src & AXP_WAKEUP_GPIO1_RAISING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio1 raising irq enable*/
			reg_val |= (0x1 << 7);
	...
	if(wakeup_src & AXP_WAKEUP_GPIO2_FALLING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}

			/* enable gpio2 edgg irq*/
			reg_val |= (0x1 << 2);
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_GPIO2CFG, &reg_val)){
				return -1;
			}
			/* enable gpio2 as input*/
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio0 falling irq enable*/
			reg_val |= (0x1 << 6);
			
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_GPIO2CFG, &reg_val)){
				return -1;
			}
	}
	
	if(wakeup_src & AXP_WAKEUP_GPIO2_RAISING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio2 raising irq enable*/
			reg_val |= (0x1 << 7);
	...
	if(wakeup_src & AXP_WAKEUP_GPIO3_FALLING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}

			/* enable gpio3 edgg irq*/
			reg_val |= (0x1 << 3);
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_GPIO3CFG, &reg_val)){
				return -1;
			}
			/* enable gpio3 as input*/
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio3 falling irq enable*/
			reg_val |= (0x1 << 6);
			
			if(twi_byte_rw(TWI_OP_WR, AXP_ADDR,AXP20_GPIO3CFG, &reg_val)){
				return -1;
			}
	}
	if(wakeup_src & AXP_WAKEUP_GPIO3_RAISING){
			if(twi_byte_rw(TWI_OP_RD, AXP_ADDR,AXP20_IRQEN5, &reg_val)){
				return -1;
			}
			...
			reg_val &= ~(0x7 << 0);
			reg_val |= (0x2 << 0);
			/* enable gpio3 raising irq enable*/
			reg_val |= (0x1 << 7);
...
{
	__u8 reg_val = 0;
	__u8 reg_power_mode = 0;
	__u8 reg_chgled = 0;

	/* config wakeup signal */
	if (twi_byte_rw_nommu(TWI_OP_RD, AXP_ADDR, 0x31, &reg_val) != 0)
		return -1;
	...
	/* Reading Power output control */
	if (twi_byte_rw_nommu(TWI_OP_RD, AXP_ADDR, 0x12, &reg_val))
		return -1;

	/* Reading Power operating mode */
	if (twi_byte_rw_nommu(TWI_OP_RD, AXP_ADDR, 0x01, &reg_power_mode))
		return -1;


	/* Don't close 3.3v if charging*/
	if (reg_power_mode & 0x40) {
		reg_val &= ~0x5a; // charging
	} else {
		reg_val &= ~0x5b; // not charging or charging has been completed
	}
