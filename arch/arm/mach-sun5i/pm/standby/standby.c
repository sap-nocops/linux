...
static void standby(struct aw_pm_info *arg);
...
static void standby(struct aw_pm_info *arg)
{
	...
	arg->key_val = * (volatile unsigned int *)(KEY_BASSADDRESS + LRADC_DATA0);
    #if(ALLOW_DISABLE_HOSC)
    ...
    pm_info.standby_para.event |= standby_query_int(INT_SOURCE_PIO)? 0:SUSPEND_WAKEUP_SRC_PIO;
