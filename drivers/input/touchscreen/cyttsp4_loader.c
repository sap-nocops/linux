...
#define CY_AUTO_FW_UPDATE_RETRY_MAX 3

#include "cyttsp4_params_amp_ara.h"
static struct touch_settings cyttsp4_sett_param_regs_amp_ara = {
	.data = (uint8_t *)&cyttsp4_param_regs_amp_ara[0],
	.size = ARRAY_SIZE(cyttsp4_param_regs_amp_ara),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_param_size_amp_ara = {
	.data = (uint8_t *)&cyttsp4_param_size_amp_ara[0],
	.size = ARRAY_SIZE(cyttsp4_param_size_amp_ara),
	.tag = 0,
};

static struct cyttsp4_touch_config cyttsp4_ttconfig_amp_ara = {
	.param_regs = &cyttsp4_sett_param_regs_amp_ara,
	.param_size = &cyttsp4_sett_param_size_amp_ara,
	.fw_ver = ttconfig_fw_ver_amp_ara,
	.fw_vsize = ARRAY_SIZE(ttconfig_fw_ver_amp_ara),
};

#include "cyttsp4_params_wtk_ara.h"
static struct touch_settings cyttsp4_sett_param_regs_wtk_ara = {
	.data = (uint8_t *)&cyttsp4_param_regs_wtk_ara[0],
	.size = ARRAY_SIZE(cyttsp4_param_regs_wtk_ara),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_param_size_wtk_ara = {
	.data = (uint8_t *)&cyttsp4_param_size_wtk_ara[0],
	.size = ARRAY_SIZE(cyttsp4_param_size_wtk_ara),
	.tag = 0,
};

static struct cyttsp4_touch_config cyttsp4_ttconfig_wtk_ara = {
	.param_regs = &cyttsp4_sett_param_regs_wtk_ara,
	.param_size = &cyttsp4_sett_param_size_wtk_ara,
	.fw_ver = ttconfig_fw_ver_wtk_ara,
	.fw_vsize = ARRAY_SIZE(ttconfig_fw_ver_wtk_ara),
};

#include "cyttsp4_params_amp_leo.h"
static struct touch_settings cyttsp4_sett_param_regs_amp_leo = {
	.data = (uint8_t *)&cyttsp4_param_regs_amp_leo[0],
	.size = ARRAY_SIZE(cyttsp4_param_regs_amp_leo),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_param_size_amp_leo = {
	.data = (uint8_t *)&cyttsp4_param_size_amp_leo[0],
	.size = ARRAY_SIZE(cyttsp4_param_size_amp_leo),
	.tag = 0,
};

static struct cyttsp4_touch_config cyttsp4_ttconfig_amp_leo = {
	.param_regs = &cyttsp4_sett_param_regs_amp_leo,
	.param_size = &cyttsp4_sett_param_size_amp_leo,
	.fw_ver = ttconfig_fw_ver_amp_leo,
	.fw_vsize = ARRAY_SIZE(ttconfig_fw_ver_amp_leo),
};

#include "cyttsp4_params_gis_leo.h"
static struct touch_settings cyttsp4_sett_param_regs_gis_leo = {
	.data = (uint8_t *)&cyttsp4_param_regs_gis_leo[0],
	.size = ARRAY_SIZE(cyttsp4_param_regs_gis_leo),
	.tag = 0,
};

static struct touch_settings cyttsp4_sett_param_size_gis_leo = {
	.data = (uint8_t *)&cyttsp4_param_size_gis_leo[0],
	.size = ARRAY_SIZE(cyttsp4_param_size_gis_leo),
	.tag = 0,
};

static struct cyttsp4_touch_config cyttsp4_ttconfig_gis_leo = {
	.param_regs = &cyttsp4_sett_param_regs_gis_leo,
	.param_size = &cyttsp4_sett_param_size_gis_leo,
	.fw_ver = ttconfig_fw_ver_gis_leo,
	.fw_vsize = ARRAY_SIZE(ttconfig_fw_ver_gis_leo),
};
		...
		struct cyttsp4_dev_id *dev_id)
{
	...
	/* workaround for manual FW update easily fail */
	msleep(200);
...
#if CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP4_BINARY_FW_UPGRADE
...
static int cyttsp4_check_ttconfig_version_panel(struct cyttsp4_device *ttsp)
{
	struct device *dev = &ttsp->dev;
	struct cyttsp4_loader_data *data = dev_get_drvdata(dev);
	struct cyttsp4_touch_config *ttconfig;
	...
	case CY_PANEL_ID_AMP_ARA:
		ttconfig = &cyttsp4_ttconfig_amp_ara;
		break;
	case CY_PANEL_ID_WTK_ARA:
		ttconfig = &cyttsp4_ttconfig_wtk_ara;
		break;
	case CY_PANEL_ID_AMP_LEO:
		ttconfig = &cyttsp4_ttconfig_amp_leo;
		break;
	case CY_PANEL_ID_GIS_LEO:
		ttconfig = &cyttsp4_ttconfig_gis_leo;
		break;
	default:
		ttconfig = NULL;
		break;
		...
		void *context)
{
	...
	if (upgrade) {
		...
		upgrade = cyttsp4_check_ttconfig_version_panel(ttsp);
		if (upgrade) {
			_cyttsp4_firmware_cont(fw, ttsp);
			data->builtin_bin_fw_status = 0;
			complete(&data->builtin_bin_fw_complete);
...
upgrade:
	if (data->si == NULL) {
		dev_err(dev, "%s: Fail get sysinfo pointer from core, use default panel_id = %d\n",
				__func__, CY_PANEL_ID_AMP_ARA);
		panel_id = CY_PANEL_ID_AMP_ARA;
	} else {
		panel_id=data->si->si_ptrs.pcfg->panel_info0;
		dev_dbg(dev, "%s: panel_id = %d\n", __func__, panel_id);
	}

	switch (panel_id){
	case CY_PANEL_ID_AMP_ARA:
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_FW_FILE_NAME_AMP_ARA, dev, GFP_KERNEL, ttsp,
				_cyttsp4_firmware_cont_builtin);
		break;
	case CY_PANEL_ID_WTK_ARA:
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_FW_FILE_NAME_WTK_ARA, dev, GFP_KERNEL, ttsp,
				_cyttsp4_firmware_cont_builtin);
		break;
	case CY_PANEL_ID_AMP_LEO:
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_FW_FILE_NAME_AMP_LEO, dev, GFP_KERNEL, ttsp,
				_cyttsp4_firmware_cont_builtin);
		break;
	case CY_PANEL_ID_GIS_LEO:
		retval = request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
				CY_FW_FILE_NAME_GIS_LEO, dev, GFP_KERNEL, ttsp,
				_cyttsp4_firmware_cont_builtin);
		break;

	default:
		dev_err(dev, "%s: No this vendor, panel_id=%d\n",
			__func__, panel_id);
		break;
	}
	...
	fwupdate_counter++;
	dev_dbg(dev, "%s: FW binary upgrade finished, fwupdate_counter = %d\n", __func__, fwupdate_counter);

	pm_runtime_get(dev);
	data->si = cyttsp4_request_sysinfo(ttsp);
	pm_runtime_put(dev);

	if (data->si == NULL || panel_id != data->si->si_ptrs.pcfg->panel_info0) {
		dev_err(dev, "%s: last upgrade failed or updated wrong FW, need upgrade again\n", __func__);
		if(fwupdate_counter < CY_AUTO_FW_UPDATE_RETRY_MAX)
			goto upgrade;
	} else {
		dev_dbg(dev, "%s: FW binary upgrade completed with right FW\n", __func__);
		fwupdate_counter = 0;
	}
		...
		struct work_struct *fw_and_config_upgrade)
{
	...
	struct cyttsp4_device *ttsp = data->ttsp;
	struct device *dev = &ttsp->dev;
	bool invalid_touch_app = false;

	/* check sysinfo */
	data->si = cyttsp4_request_sysinfo(ttsp);
	if (data->si == NULL)
		dev_err(dev, "%s: Fail get sysinfo pointer from core - FW maybe corrupted\n", __func__);

	/* check fw corruption */
	invalid_touch_app = cyttsp4_request_fw_status(ttsp);
	if(invalid_touch_app) {
		dev_err(dev, "%s: FW corrupted! Skip FW recovery\n", __func__);
		return;
	} else {
		dev_info(dev, "%s: Panel has valid FW inside, skip FW update\n", __func__);
		return;
	}
...
static int cyttsp4_loader_init(void)
{
	int rc = 0;
	int i, j;
...
static void cyttsp4_loader_init_async(void *unused,
...
static int __init cyttsp4_loader_init_schedule(void)
{
	async_schedule_domain(cyttsp4_loader_init_async, NULL,
			&cyttsp4_async_domain);
	return 0;
}
module_init(cyttsp4_loader_init_schedule);
