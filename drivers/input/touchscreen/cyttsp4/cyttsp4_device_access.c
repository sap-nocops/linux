	...
	switch(flag){
		...
		case CY_CALI_SHOW_IDAC:		
			...
			for(i = 0; i < sizeof(idac_buf); i++) {
				printptr += sprintf(printptr, "0x%02x ", idac_buf[i]);
				if((i+1) % 10 == 0)
					printptr += sprintf(printptr, "\n");
			}
			printk(KERN_INFO "%512s", printbuf);
			printk(KERN_INFO "%512s", printbuf+512);
			printk(KERN_INFO "%512s", printbuf+1024);
			printk(KERN_INFO "%512s", printbuf+1536);
			printk(KERN_INFO "%512s", printbuf+2048);
...
static int cyttsp4_open_test(struct cyttsp4_device_access_data *dad)
{
	...
	rds_cmd[3] = 0x01; //len high bits
	rds_cmd[4] = 0x2D; //len low bits
