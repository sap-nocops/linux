...
#define cyevd_bookeen

int touch_state = 0;

void set_touch_state(int state)
{
	touch_state = state;
}
...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_tch_rec)
{
	...
	for (i = 0; i < num_cur_tch; i++) {
		...
		/* get touch info */
		cyttsp4_get_touch(md, &tch, si->xy_data +(i * si->si_ofs.tch_rec_size));

		input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, 8);

		input_report_abs(md->input, ABS_MT_TRACKING_ID, i+1);

		/* all devices: position and pressure fields */
		for (j = 0; j <= CY_ABS_W_OST ; j++) {
			sig = md->pdata->frmwrk->abs[((CY_ABS_X_OST + j) *CY_NUM_ABS_SET) + 0];
			if (sig != CY_IGNORE_VALUE){
				if(sig == ABS_MT_POSITION_X)
					input_report_abs(md->input, ABS_MT_POSITION_Y,758 - tch.abs[CY_TCH_X + j]);
				else if(sig == ABS_MT_POSITION_Y)
					input_report_abs(md->input, ABS_MT_POSITION_X,tch.abs[CY_TCH_X + j]);
			}
		}
	...
	if (num_cur_tch == 0) {
		input_report_abs(md->input, ABS_MT_TOUCH_MAJOR, 0);
...
*/
	...
	md->mt_function.input_report(md->input, ABS_MT_TOUCH_MAJOR, 0); 
