...
/* Initial grip suppression settings */
...
static irqreturn_t cyttsp4_irq(int irq, void *handle)
{
	...
	/* acknowledge interrupt (specific to A13) */
	if (!cd->pdata->judge_int_occur()) {
		cd->pdata->clear_penirq();
	}
	else {
	    return IRQ_NONE;
	}
	...
	if (IS_BOOTLOADER(mode[0], mode[1])) {
		...
		dev_info(dev, "%s: restart switch to bl m=%d -> m=%d\n",
...
static void cyttsp4_watchdog_work(struct work_struct *work)
{
	...
	int mode;
	int retval;
	static int startup_fail_count = 5;
	...
	if (IS_BOOTLOADER(mode, 0) && IS_BOOTLOADER(rep_stat, 0)) {
		dev_err(cd->dev,
		...
		u8 new_dev_mode, int new_op_mode)
{
	...
	do {		
		...
		dev_err(cd->dev, "%s: back from wait t=%ld cd->mode=%d\n",
				__func__, t, cd->mode);

retry:
		retry--;
	}while( cd->mode != new_dev_mode && (cd->mode & CY_HST_MODE_CHANGE) && retry);
...
static int cyttsp4_grip_set_param(struct cyttsp4_core_data* cd, struct cyttsp4_grip_suppression_data* data)
{
	...
	printk(KERN_DEBUG "cyttsp4:setting grip suppression %d %d %d %d %d %d %d %d %d", 
		...
		struct device_attribute *attr, char *buf)
{
	...
	for ( ;cmd <= CY_OPT_PARAM_GRP_INV; cmd++) {
		...
		pbuf += sprintf(pbuf, "0x%02x 0x%04x ", cmd, data);
	}
	pm_runtime_put(dev);
...
static int cyttsp4_core_wake_(struct cyttsp4_core_data *cd)
{
	...
	{
		//wake up the TP
		GTP_GPIO_OUTPUT(gtp_power_gpio_18, "ctp_powerpin_18", 1);
	}
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	rc = cd->pdata->set_irq_mode(CTP_IRQ_MODE);
	if (rc != 0){
...
void cyttsp4_shutdown(struct cyttsp4_core *core)
{
	if( 0 == use_power_pin )
	{
		printk("[cyttsp4] Shutdown\n");
		GTP_GPIO_OUTPUT(gtp_power_gpio_18, "ctp_powerpin_18", 0);
...
static int __init cyttsp4_core_init(void)
{
	...
	{
		GTP_GPIO_OUTPUT(gtp_power_gpio_18, "ctp_powerpin_18", 1);
		GTP_GPIO_OUTPUT(gtp_power_gpio_33, "ctp_powerpin_33", 0);	//Enable 3.3V
	}
