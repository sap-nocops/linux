...
#define CY_CORE_WAIT_SYSINFO_MODE_TIMEOUT	600
...
static const u8 ldr_exit_wo_crc_check[] = {
...
/*
 * returns touch fw app status
 */
static bool cyttsp4_request_fw_status_(
		struct cyttsp4_device *ttsp)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);
	...
	mutex_lock(&cd->system_lock);
	invalid_touch_app = cd->invalid_touch_app;
	mutex_unlock(&cd->system_lock);

	return invalid_touch_app;
}

static void cyttsp4_request_set_power_state_(
		struct cyttsp4_device *ttsp, u8 state)
{
	struct cyttsp4_core *core = ttsp->core;
	struct cyttsp4_core_data *cd = dev_get_drvdata(&core->dev);

	mutex_lock(&cd->system_lock);
	cd->power_state = state;
	dev_err(&core->dev, "%s: power_state=%d\n", __func__, cd->power_state);
	mutex_unlock(&cd->system_lock);
}
		...
		u16 *version)
{
	...
	*version = GET_FIELD16(si, &data[CY_TTCONFIG_VERSION_OFFSET]);
	...
	/* blver_minor: 0 - no panel id in BL, only available in ttconfig version; non-zero, panel ID in BL: 
					1 - Amphenol ARA; 2 - Wintek ARA; 0x10 - Amphenol LEO; 0x11 - GIS LEO */
	dev_dbg(cd->dev, "%s: blver_minor: 0x%02X\n", __func__, si->si_ptrs.cydata->blver_minor);
	switch(si->si_ptrs.cydata->blver_minor) {
	case 0:
		si->si_ptrs.pcfg->panel_info0 = data[CY_TTCONFIG_VERSION_OFFSET+1];
		break;
	case 1:
		si->si_ptrs.pcfg->panel_info0 = CY_PANEL_ID_AMP_ARA;
		break;
	case 2:
		si->si_ptrs.pcfg->panel_info0 = CY_PANEL_ID_WTK_ARA;
		break;
	case CY_PANEL_ID_AMP_LEO:
		si->si_ptrs.pcfg->panel_info0 = CY_PANEL_ID_AMP_LEO;
		break;
	case CY_PANEL_ID_GIS_LEO:
		si->si_ptrs.pcfg->panel_info0 = CY_PANEL_ID_GIS_LEO;
		break;

	default:
		dev_err(cd->dev, "%s: unknown panel id\n", __func__);
		break;
	}
		...
		int timeout_ms)
{
	...
	if (cd->pdata->power) {
		...
		dev_dbg(dev, "%s: wake up HW\n", __func__);
		rc = cd->pdata->power(cd->pdata, CY_POWER_WAKEUP, dev, &cd->ignore_irq);
	} else {
		/* Initiate a read transaction to wake up */
		rc = cyttsp4_adap_read(cd, CY_REG_BASE, &mode, sizeof(mode));
	}
	if (rc < 0) {
		dev_err(dev, "%s: HW wake up fails r=%d\n", __func__, rc);
		/* Initiate another read transaction to wake up */
		rc = cyttsp4_adap_read(cd, CY_REG_BASE, &mode, sizeof(mode));
	} else
		dev_vdbg(cd->dev, "%s: HW wake up succeeds\n", __func__);
	mutex_unlock(&cd->system_lock);
	...
	mutex_lock(&cd->system_lock);
	if (IS_TMO(t)) {
		dev_dbg(dev, "%s: TMO waiting for wake up\n", __func__);
		cd->int_status &= ~CY_INT_AWAKE;
...
static int cyttsp4_startup_from_suspend(struct cyttsp4_core_data *cd)
{
	int retry = CY_CORE_STARTUP_RETRY_COUNT;
	int rc;
	bool detected = false;
...
reset:
	...
	/* reset hardware & wait for heartbeat */
	rc = cyttsp4_reset_and_wait(cd);
	if (rc < 0) {
		dev_err(cd->dev, "%s: Error on h/w reset r=%d\n", __func__, rc);
		RETRY_OR_EXIT(retry--, reset, exit);
	...
	/* try ldr_exit_wo_crc_check first, if failed, try ldr_exit */
	if(retry == CY_CORE_STARTUP_RETRY_COUNT) {
		rc = cyttsp4_adap_write(cd, CY_REG_BASE, (u8 *)ldr_exit_wo_crc_check,
				sizeof(ldr_exit_wo_crc_check));
	} else {
		rc = cyttsp4_adap_write(cd, CY_REG_BASE, (u8 *)ldr_exit,
				sizeof(ldr_exit));
	}
...
static int cyttsp4_core_rt_suspend(struct device *dev)
{
	...
	if(cd->power_state == CY_POWER_OFF){
		...
		cyttsp4_stop_wd_timer(cd);
		/* power OFF by switching off power switch */
		if (cd->pdata->power) {
			dev_dbg(cd->dev, "%s: Power down HW\n", __func__);
			rc = cd->pdata->power(cd->pdata, CY_POWER_OFF, cd->dev, &cd->ignore_irq);
		} else {
			dev_dbg(cd->dev, "%s: No power function\n", __func__);
			rc = 0;
		}
...
static int cyttsp4_core_rt_resume(struct device *dev)
{
	...
	if(cd->power_state == CY_POWER_OFF){
		if (cd->sysinfo.ready) {
			/* power ON by switching on power switch */
			if (cd->pdata->power) {
				rc = cd->pdata->power(cd->pdata, CY_POWER_ON, cd->dev, &cd->ignore_irq);
			} else {
				dev_dbg(cd->dev, "%s: No power function\n", __func__);
				rc = 0;
			}
			if (rc < 0) {
				dev_err(cd->dev, "%s: HW Power up fails r=%d\n",
						__func__, rc);
				return -EINVAL;
			}

			rc = cyttsp4_startup_from_suspend(cd);
			if (rc < 0)
				dev_err(cd->dev, "%s: Failed, r=%d\n",
					__func__, rc);

		} else
			dev_dbg(cd->dev, "%s: sysinfo not ready, abort resume\n", __func__);
	}
	else {
 ...
 * Show Firmware version via sysfs
 */
static ssize_t cyttsp4_panel_info_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	...
	return snprintf(buf, CY_MAX_PRBUF_SIZE,
		"TrueTouch Panel ID: 0x%02X\n",	cd->sysinfo.si_ptrs.pcfg->panel_info0);
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(panel_id, S_IRUGO, cyttsp4_panel_info_show, NULL),
...
static int cyttsp4_core_probe(struct cyttsp4_core *core)
{
	...
	rc = input_arbiter_init();
	if (rc < 0) {
		dev_err(dev, "%s: Error, fail init input arbiter\n", __func__);
		goto error_startup;
	}
...
static struct cyttsp4_core_driver cyttsp4_core_driver = {
	...
	.request_fw_status = cyttsp4_request_fw_status_,	
	...
	.request_set_power_state = cyttsp4_request_set_power_state_,
