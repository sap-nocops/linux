...
cyttsp4_get_mt_touches_pr_tch:
	...
	input_arbiter_report_touch_event(md->input,tch.abs[CY_TCH_E]);
	return;
}
...
static int cyttsp4_xy_worker(struct cyttsp4_mt_data *md)
{
	...
	if (IS_BAD_PKT(rep_stat)) {
		dev_vdbg(dev, "%s: Invalid buffer detected\n", __func__);
		rc = 0;
		goto cyttsp4_xy_worker_exit;
	}
				...
				unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		...
		else if ((*blank == FB_BLANK_POWERDOWN) && (md->is_screen_on)) {
			cyttsp4_mt_disable(&(md->ttsp->dev));
			md->is_screen_on = false;
		}
	}
 ...
 * new kernel versions
 */
static int cyttsp4_mt_pm_runtime_force_suspend(struct device *dev)
{
	int ret = 0;
...
{
	struct cyttsp4_mt_data *md = dev_get_drvdata(dev);
	struct cyttsp4_device *ttsp = md->ttsp;
...
static int cyttsp4_setup_input_device(struct cyttsp4_device *ttsp)
{
	...
	for (i = 0; i < (md->pdata->frmwrk->size / CY_NUM_ABS_SET); i++) {
		...
		if (signal != CY_IGNORE_VALUE) {
			...
			dev_vdbg(dev, "%s: register signal=%02X min=%d max=%d\n",
