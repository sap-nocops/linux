...
static int cyttsp4_btn_enable(struct device *dev);
	...
	int btn_no, int btn_state)
{
	...
	si->btn[btn_no].state = btn_state;
	input_arbiter_report_key_event(bd->input, si->btn[btn_no].key_code, btn_state);
...
static int cyttsp4_xy_worker(struct cyttsp4_btn_data *bd)
{
	...
	if (IS_LARGE_AREA(tt_stat)) {
		dev_vdbg(dev, "%s: Large area detected\n", __func__);
		/* Do not report button when large object detected */
		return 0;
	}
				...
				unsigned long event, void *data)
{
	...
	int *blank;
	struct cyttsp4_btn_data *bd =
		container_of(self, struct cyttsp4_btn_data, fb_notif);
	struct device *dev = &bd->ttsp->dev;
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK) {
		blank = evdata->data;
		dev_info(dev, "%s, blank: %d, is_screen_on: %d\n", __func__, *blank, bd->is_screen_on);
		if ((*blank == FB_BLANK_UNBLANK) && (!bd->is_screen_on)) {
			cyttsp4_btn_enable(&(bd->ttsp->dev));
			bd->is_screen_on = true;
		}
		else if ((*blank == FB_BLANK_POWERDOWN) && (bd->is_screen_on)) {
			cyttsp4_btn_disable(&(bd->ttsp->dev));
			bd->is_screen_on = false;
		}
	}

	return 0;
}
static void cyttsp4_btn_setup_early_suspend(struct cyttsp4_btn_data *bd)
{
	int retval = 0;
	struct device *dev = &bd->ttsp->dev;
	...
	retval = fb_register_client(&bd->fb_notif);
	if (retval)
		dev_err(&bd->ttsp->dev,
...
#else
static void cyttsp4_btn_setup_early_suspend(struct cyttsp4_mt_data *md)
{
	return;
}
#endif
...
static int cyttsp4_btn_rt_resume(struct device *dev)
{
	struct cyttsp4_btn_data *bd = dev_get_drvdata(dev);
	dev_dbg(dev, "%s\n", __func__);
 ...
 * new kernel versions
 */
static int cyttsp4_btn_pm_runtime_force_suspend(struct device *dev)
{
	int ret = 0;
	...
	ret = cyttsp4_btn_rt_suspend(dev);
...
static int cyttsp4_btn_suspend(struct device *dev)
{
	...
	/* screen cannot be on in suspend */
	bd->is_screen_on = false;
...
{
	struct cyttsp4_btn_data *bd = dev_get_drvdata(dev);
	struct cyttsp4_device *ttsp = bd->ttsp;

	dev_dbg(dev, "%s\n", __func__);
	cyttsp4_request_set_power_state(ttsp, CY_POWER_OFF);
	pm_runtime_put(dev);
	return 0;
}
...
#else
static const struct dev_pm_ops cyttsp4_btn_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(cyttsp4_btn_suspend, cyttsp4_btn_resume)
	SET_RUNTIME_PM_OPS(cyttsp4_btn_rt_suspend, cyttsp4_btn_rt_resume, NULL)
};
#endif
...
#elif CONFIG_FB
	cyttsp4_btn_setup_early_suspend(bd);
#endif
