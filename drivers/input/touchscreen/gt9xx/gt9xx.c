...
#define GTP_IRQ_NO		(gpioinfo[0].port_num)
...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    input_report_abs(ts->input_dev, ABS_MT_TRACKING_ID, id+1);
#if GTP_CUSTOM_CFG
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (x*GTP_MAX_WIDTH)/ts->abs_x_max);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (y*GTP_MAX_HEIGHT)/ts->abs_y_max);
#else 
    input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
    input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
#endif
...
void gtp_int_sync(s32 ms)
{
    GTP_GPIO_OUTPUT(gtp_int_gpio, "ctp_int_port", 0);
    msleep(ms);
    GTP_GPIO_AS_INT(gtp_int_gpio, "ctp_int_port");
}
...
//    gpio_set_one_pin_io_status(gtp_rst_gpio, 1, "ctp_reset");
//    gpio_write_one_pin_value(gtp_rst_gpio, 0, "ctp_reset");

    if (use_power_pin)
    {   
        GTP_GPIO_OUTPUT(gtp_power_gpio, "ctp_powerpin_18", 0);
        msleep(ms);
        GTP_GPIO_OUTPUT(gtp_power_gpio, "ctp_powerpin_18", 1);
    }
	
    GTP_GPIO_OUTPUT(gtp_rst_gpio, "ctp_reset" , 0);   // begin select I2C slave addr
    msleep(ms);                         // T2: > 10ms
    // HIGH: 0x28/0x29, LOW: 0xBA/0xBB
//    gpio_set_one_pin_io_status(gtp_int_gpio, 1, "ctp_int_port");
//    gpio_write_one_pin_value(gtp_int_gpio, client->addr == 0x14, "ctp_int_port");
    GTP_GPIO_OUTPUT(gtp_int_gpio, "ctp_int_port", client->addr == 0x14);

    msleep(2);                          // T3: > 100us
//   gpio_set_one_pin_io_status(gtp_rst_gpio, 1, "ctp_reset");
//   gpio_write_one_pin_value(gtp_rst_gpio, 1, "ctp_reset");
    GTP_GPIO_OUTPUT(gtp_rst_gpio, "ctp_reset", 1);
    
    msleep(6);                          // T4: > 5ms

//    gpio_set_one_pin_io_status(gtp_rst_gpio,0, "ctp_reset");
    GTP_GPIO_AS_INPUT(gtp_rst_gpio, "ctp_reset");    // end select I2C slave addr
...
static s8 gtp_wakeup_sleep(struct goodix_ts_data * ts)
{
    ...
    if (CHIP_TYPE_GT9F == ts->chip_type)
    {
        ...
        GTP_GPIO_OUTPUT(gtp_int_gpio, "ctp_int_port", 1);
        msleep(5);
...
static int ctp_fetch_sysconfig_para(void)
{
    ...
    int temp_screen_max_x=0;
    int temp_screen_max_y=0;
    ...
    if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp_screen_max_x", &temp_screen_max_x, 1))
    {
        pr_err("%s: script_parser_fetch err. \n", __func__);
        ret = -1;
    }
    pr_info("%s: screen_max_x = %d. \n", __func__, temp_screen_max_x);

    if(SCRIPT_PARSER_OK != script_parser_fetch("ctp_para", "ctp_screen_max_y", &temp_screen_max_y, 1))
    {
        pr_err("%s: script_parser_fetch err. \n", __func__);
        ret = -1;
    }
    pr_info("%s: screen_max_y = %d. \n", __func__, temp_screen_max_y);

    screen_max_x=(u16)temp_screen_max_x;
    screen_max_y=(u16)temp_screen_max_y;
...
GTP_DEBUG("1490 : GTP_MAX_WIDTH: %d, GTP_MAX_HEIGHT: %d",GTP_MAX_WIDTH,GTP_MAX_HEIGHT);
#if GTP_CUSTOM_CFG
    config[RESOLUTION_LOC]     = (u8)GTP_MAX_HEIGHT;
    config[RESOLUTION_LOC + 1] = (u8)(GTP_MAX_HEIGHT>>8);
    config[RESOLUTION_LOC + 2] = (u8)GTP_MAX_WIDTH;
    config[RESOLUTION_LOC + 3] = (u8)(GTP_MAX_WIDTH>>8);
    ...
    if ((ts->abs_x_max == 0) && (ts->abs_y_max == 0))
    {
        ...
        ts->int_trigger_type = GTP_INT_TRIGGER; //(config[TRIGGER_LOC]) & 0x03;     //Config is not good.
    }
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
    gtp_power_gpio = gpio_request_ex("ctp_para", "ctp_powerpin_18");
    if (!gtp_power_gpio) 
    {
        GTP_ERROR("Failed to request GPIO:ctp_power_port, ERRNO:%d", (s32)gtp_power_gpio);
        GTP_GPIO_FREE(gtp_power_gpio);
        use_power_pin=0;
    }
    else 
    {
    ...
    gtp_int_gpio = GTP_GPIO_REQUEST("ctp_int_port");
    if (!gtp_int_gpio) 
    {
        GTP_ERROR("Failed to request GPIO:ctp_int_port, ERRNO:%d", (s32)gtp_int_gpio);
        ret = -ENODEV;
    }
    else
    {
    ...
    gtp_rst_gpio = GTP_GPIO_REQUEST("ctp_reset");
    if (!gtp_rst_gpio) 
    {
        GTP_ERROR("Failed to request GPIO:ctp_reset, ERRNO:%d",(s32)gtp_rst_gpio,ret);
        ret = -ENODEV;
    }

    GTP_GPIO_AS_INPUT(gtp_rst_gpio, "ctp_reset");
...
/*******************************************************
Function:
    get irq status (sun5i).
Output:
    Executive outcomes.
        0: irq occur, -1: irq not occur.
*******************************************************/
static int gtp_judge_int_occur(void)
{
    int reg_val;
    int ret = -1;
    reg_val = readl(gpio_addr + PIO_INT_STAT_OFFSET);
    if(reg_val&(1<<(GTP_IRQ_NO))){
	ret = 0;
    }
    return ret; 	
}
...
/*******************************************************
Function:
    acknowledge interrupt (sun5i).
Output:
    Executive outcomes.
        0: succeed, -1: failed.
*******************************************************/
static int gtp_clear_penirq(void)
{
    int reg_val;

    reg_val = readl(gpio_addr + PIO_INT_STAT_OFFSET);
    reg_val = reg_val|(1<<(GTP_IRQ_NO));
...
Output:
    Executive outcomes.
        0: succeed, -1: failed.
*******************************************************/
static int gtp_set_irq_mode(ext_int_mode irq_mode)
{
    int ret = 0;
    int reg_value;
    u32 reg_num = 0;
    ...
    {
	GTP_GPIO_GET_INFO(gtp_int_gpio, gpioinfo, "ctp_int_port");
	GTP_INFO("%s, %d: gpio_int_info, port = %d, port_num = %d. \n", __func__, __LINE__,gpioinfo[0].port, gpioinfo[0].port_num);
    }

    reg_num 	= (gpioinfo[0].port_num)%8;
    reg_addr 	= (gpioinfo[0].port_num)/8;
    reg_val 	= readl(gpio_addr + int_cfg_addr[reg_addr]);
    reg_val 	&= (~(7 << (reg_num * 4)));
    reg_val 	|= (irq_mode << (reg_num * 4));
    writel(reg_val,gpio_addr+int_cfg_addr[reg_addr]);                                         

    reg_value = readl(gpio_addr + PIO_INT_STAT_OFFSET);
    if ((reg_value = (reg_value&(1<<(GTP_IRQ_NO))))) {
	writel(reg_value,gpio_addr + PIO_INT_STAT_OFFSET);
     }
     
    reg_val = readl(gpio_addr+PIO_INT_CTRL_OFFSET); 
    reg_val |= (1 << (gpioinfo[0].port_num));
...
static s8 gtp_request_irq(struct goodix_ts_data *ts)
{
    ...
    ret = gtp_set_irq_mode(NEGATIVE_EDGE);
    if (ret != 0)
    {
	GTP_ERROR("set irq mode failed!\n");
	return -1;
    }
    ...
    if (ret)
    {
        ...
        GTP_GPIO_AS_INPUT(gtp_int_gpio, "ctp_int_port");
        GTP_GPIO_FREE(gtp_int_gpio);
...
GTP_DEBUG("2005: GTP_MAX_WIDTH: %d, GTP_MAX_HEIGHT: %d",GTP_MAX_WIDTH,GTP_MAX_HEIGHT);
#if GTP_CUSTOM_CFG
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, GTP_MAX_WIDTH, 0, 0);
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0, GTP_MAX_HEIGHT, 0, 0);
#else
...
int goodix_ts_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	struct i2c_adapter *adapter = client->adapter;

	if (twi_id == adapter->nr) {
		printk("%s: Detected chip %s at adapter %d, address 0x%02x\n",
			 __func__, GTP_I2C_NAME, i2c_adapter_id(adapter), client->addr);

		strcpy(info->type, GTP_I2C_NAME);
		return 0;
	} else {
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
    ...
    if (ts->use_irq)
    {
	    gtp_set_irq_mode(NEGATIVE_EDGE);
        gtp_irq_enable(ts);
    }
    else
    {
...
static int gtp_pm_suspend(struct device *dev)
{
	...
	/* enter deep-sleep only on PM_SUSPEND_MEM or PM_SUSPEND_BOOTFAST */
...
static struct dev_pm_ops gtp_pm_ops = {
	SET_SYSTEM_SLEEP_PM_OPS(gtp_pm_suspend, gtp_pm_resume)
};
...
MODULE_AUTHOR("Juba BELGAÏD <jubab@bookeen.com>");
MODULE_AUTHOR("Alexis Huet <alexish@bookeen.com>");
