...
static struct input_dev *wakeup_dev;
...
static irqreturn_t mpwr_gpio_isr(int irq, void *dev_id)
{
	...
	if (irq == mpwr->wakeup_irq) {
		...
		input_report_key(wakeup_dev, KEY_WAKEUP, 1);
		input_sync(wakeup_dev);
		input_report_key(wakeup_dev, KEY_WAKEUP, 0);
		input_sync(wakeup_dev);
...
static int __init mpwr_driver_init(void)
{
	...
	wakeup_dev = input_allocate_device();
	if (!wakeup_dev) {
		ret = -ENOMEM;
		goto err_wakeup;
	}

	wakeup_dev->name = "modem-power wakeup key";
	wakeup_dev->evbit[0] = BIT_MASK(EV_KEY);
	wakeup_dev->keybit[BIT_WORD(KEY_WAKEUP)] = BIT_MASK(KEY_WAKEUP);

	ret = input_register_device(wakeup_dev);
	if (ret)
		goto err_wakeup;
...
{
	input_unregister_device(wakeup_dev);
	serdev_device_driver_unregister(&mpwr_serdev_driver);
