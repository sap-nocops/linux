...
#include  "../../manager/usb_msg_center.h"
...
static struct sw_hcd *g_sw_hcd0 = NULL, *c_sw_hcd = NULL;
...
static ssize_t
sw_hcd_mode_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sw_hcd	*sw_hcd	= dev_to_sw_hcd(dev);
	unsigned long	 flags;
	int		 ret	= -EINVAL;
	enum usb_role	 role;
...
static ssize_t
sw_hcd_mode_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t n)
{
	...
	if (sysfs_streq(buf, "host"))
		status = sw_hcd_platform_set_mode(sw_hcd, SW_HCD_HOST);
	else if (sysfs_streq(buf, "peripheral"))
		status = sw_hcd_platform_set_mode(sw_hcd, SW_HCD_PERIPHERAL);
	else if (sysfs_streq(buf, "otg"))
		status = sw_hcd_platform_set_mode(sw_hcd, SW_HCD_OTG);
	else
		status = -EINVAL;
	spin_unlock_irqrestore(&sw_hcd->lock, flags);
...
static DEVICE_ATTR(mode, 0644, sw_hcd_mode_show, sw_hcd_mode_store);

#if 0
static ssize_t
sw_hcd_vbus_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t n)
{
#if 0
	struct sw_hcd	*sw_hcd = dev_to_sw_hcd(dev);
	...
	sw_hcd->a_wait_bcon = val ? max_t(int, val, OTG_TIME_A_WAIT_BCON) : 0 ;
	if (sw_hcd->xceiv->state == OTG_STATE_A_WAIT_BCON)
		sw_hcd->is_active = 0;
	sw_hcd_platform_try_idle(sw_hcd, jiffies + msecs_to_jiffies(val));
	spin_unlock_irqrestore(&sw_hcd->lock, flags);
...
static ssize_t
sw_hcd_vbus_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct sw_hcd	*sw_hcd = dev_to_sw_hcd(dev);
	unsigned long	flags;
	unsigned long	val;
	...
	spin_lock_irqsave(&sw_hcd->lock, flags);
	val = sw_hcd->a_wait_bcon;
	/* FIXME get_vbus_status() is normally #defined as false...
	 * and is effectively TUSB-specific.
	 */
//	vbus = sw_hcd_platform_get_vbus_status(sw_hcd);
	vbus = 1;
	spin_unlock_irqrestore(&sw_hcd->lock, flags);
...
static DEVICE_ATTR(vbus, 0644, sw_hcd_vbus_show, sw_hcd_vbus_store);
#endif

static struct attribute *sw_hcd_attributes[] = {
	&dev_attr_mode.attr,
//	&dev_attr_vbus.attr,
	NULL
};

static const struct attribute_group sw_hcd_attr_group = {
	.attrs = sw_hcd_attributes,
};
...
static void sw_hcd_irq_work(struct work_struct *data)
{
	...
	static enum usb_role old_role = -1;
	enum usb_role role;
...
void sw_hcd_schedule_irq_work(void)
{
	if (c_sw_hcd == NULL)
		return;

	schedule_work(&c_sw_hcd->irq_work);
}
EXPORT_SYMBOL(sw_hcd_schedule_irq_work);
...
#ifdef CONFIG_SYSFS
	sysfs_remove_group(&sw_hcd->controller->kobj, &sw_hcd_attr_group);
#endif
...
#ifdef CONFIG_SYSFS
	status = sysfs_create_group(&sw_hcd->controller->kobj, &sw_hcd_attr_group);
	if (status)
		goto fail2;
#endif
