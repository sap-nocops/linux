...
static const struct rotate_format *rotate_find_format(u32 pixelformat)
{
	unsigned int i;
...
static void rotate_device_run(void *priv)
{
	...
	bpl = ctx->src_fmt.bytesperline;

	rotate_write(dev, ROTATE_IN_SIZE, ROTATE_SIZE(width, height));

	buf_addr = vb2_dma_contig_plane_dma_addr(&src->vb2_buf, 0);
	...
	bpl = ctx->dst_fmt.bytesperline;

	rotate_write(dev, ROTATE_OUT_SIZE, ROTATE_SIZE(width, height));

	buf_addr = vb2_dma_contig_plane_dma_addr(&dst->vb2_buf, 0);
	for (i = 0; i < fmt->planes; i++) {
		addr[i] = buf_addr;
		pitch[i] = bpl;
		if (i > 0)
...
static int rotate_enum_fmt(struct file *file, void *priv,
			   struct v4l2_fmtdesc *f)
{
	if (f->index < ARRAY_SIZE(rotate_formats)) {
		f->pixelformat = rotate_formats[f->index].fourcc;
				  ...
				  struct v4l2_format *f)
{
	if (!rotate_find_format(f->fmt.pix.pixelformat))
		f->fmt.pix.pixelformat = rotate_formats[0].fourcc;
				...
				struct v4l2_format *f)
{
	...
	/*
	 * Capture queue has to be checked also, because format and size
	 * depends solely on output format and size.
	 */
...
static const struct v4l2_ioctl_ops rotate_ioctl_ops = {
	...
	.vidioc_enum_fmt_vid_cap	= rotate_enum_fmt,
	...
	.vidioc_enum_fmt_vid_out	= rotate_enum_fmt,
...
static int rotate_open(struct file *file)
{
	...
	ctx->src_fmt.pixelformat = rotate_formats[0].fourcc;
	...
	ctx->dst_fmt.pixelformat = rotate_formats[0].fourcc;
	ctx->dst_fmt.field = V4L2_FIELD_NONE;
	ctx->dst_fmt.width = 640;
	ctx->dst_fmt.height = 480;
	rotate_prepare_format(&ctx->dst_fmt);
