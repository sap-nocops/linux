#include "dev_lcd.h"
#include "../disp/de_bsp/de/disp_eink_data.h"
...
int lcd_init(void)
{
    ...
    __inf("before DRV_DISP_Init!!\n");

    DRV_DISP_Init();

    __inf("before Fb_Init!!\n");

	Fb_Init(0);

    __inf("before eink init!!\n");
    
    //modified by heyihang,eink init
    eink_init_para->pixelfmt = PIXEL_MONO_8BPP;

    eink_init_para->width =((BSP_disp_get_screen_width(0) -EINK_HYNC)*4); //EINK_PANEL_W;
	eink_init_para->height=BSP_disp_get_screen_height(0) - EINK_VYNC;  //EINK_PANEL_H;

    Disp_eink_init(0, eink_init_para);
    
    //modified by heyihang,update with init mode
    Disp_eink_set_mode(0, EINK_INIT_MODE);
//    Disp_eink_update(0, 0);
