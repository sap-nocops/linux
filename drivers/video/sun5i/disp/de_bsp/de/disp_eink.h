...
#define	EINK_WAV_BUFFER_NUM		4 //24
...
#define	EINK_WAV_A2_PATH		"/system/a2.bin"
#define	EINK_WAV_A2_IN_PATH		"/system/a2_in.bin"
#define	EINK_WAV_A2_OUT_PATH		"/system/a2_out.bin"
#define	EINK_AWF_WAV_PATH		"/system/waveform.awf"
#define	EINK_VCOM_PATH			"/system/vcom"

#define	EINK_AWF_WAV_RECOVERY_PATH	"/waveform.awf"
...
#define __EINK_WRN(msg...) 				{printk(KERN_WARNING "[EINK WRN][%s] line:%d::",__func__,__LINE__);printk(msg);}
#define __EINK_ERR(msg...) 				{printk(KERN_WARNING "[EINK ERR][%s] line:%d::",__func__,__LINE__);printk(msg);}
#endif

#ifdef IS_DEBUG
#define __EINK_DBG(msg...)				{printk(KERN_WARNING "[EINK WRN][%s] line:%d::",__func__,__LINE__);printk(msg);}	
#define __EINK_LOG(msg...)				{printk(KERN_WARNING "[EINK ERR][%s] line:%d::",__func__,__LINE__);printk(msg);}
#else
...
typedef struct {
	...
	__u8 *		global_ctrl_buffer;				// wav address
	...
	__u32* 			wav_a2_data;
	__u32* 			wav_a2_in_data;
	__u32* 			wav_a2_out_data;
	__u32* 			wav_a2_data_entry;

#ifdef AWF_WAVEFORM_SUPPORTED
	char* 			p_wf_buffer;           //全文waveform指针
	__u8 wf_temp_area_tbl[C_TEMP_TBL_SIZE];
	__u32* p_init_wf;            //指向INIT mode waveform 数据???	
	...
	__u32* p_A2_wf;              //指向A2模式下waveform数据，从bin文件中读???	
	__u32* p_A2_in_wf;              //指向A2模式下waveform数据，从bin文件中读???
	__u32* p_A2_out_wf;              //指向A2模式下waveform数据，从bin文件中读???	
#endif
	...
	__u8		eink_conn_version;

	__u8		*eink_init_T_tbl;
	eink_init_wf_t	*eink_init_mode_wf;
	__u32		(*eink_ctrl_tbl_GC16)[MAX_EINK_PANEL_W];
