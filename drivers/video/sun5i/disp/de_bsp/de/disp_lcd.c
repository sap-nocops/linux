...
#define TPS61161_MAX_BRIGHTNESS   0x1F
...
//hxm add for test close backlight
bool isclosebacklight=false;
...
void LCD_get_eink_config(char *primary_key, __disp_lcd_cfg_t *lcd_cfg)
{
    ...
    //lcd_eink_main_pwr
    lcd_cfg->lcd_eink_main_pwr_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_main_pwr_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_main_pwr is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_main_pwr);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_main_pwr", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
        ...
        else
        {
            ...
            lcd_cfg->lcd_eink_main_pwr_used= 1;
        }
    }

    //lcd_eink_vneg
    lcd_cfg->lcd_eink_vneg_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_vneg_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_vneg is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_vneg);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_vneg", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_vneg not exist\n", primary_key);
        }
        else
        {
            DE_INF("%s.lcd_power gpio_port=%d,gpio_port_num:%d, data:%d\n",primary_key, gpio_info->port, gpio_info->port_num, gpio_info->data);
            lcd_cfg->lcd_eink_vneg_used= 1;
        }
    }

    //lcd_eink_vpos
    lcd_cfg->lcd_eink_vpos_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_vpos_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_vpos is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_vpos);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_vpos", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_vpos not exist\n", primary_key);
        }
        else
        {
            DE_INF("%s.lcd_power gpio_port=%d,gpio_port_num:%d, data:%d\n",primary_key, gpio_info->port, gpio_info->port_num, gpio_info->data);
            lcd_cfg->lcd_eink_vpos_used= 1;
        }
    }

    //lcd_eink_ud
    lcd_cfg->lcd_eink_ud_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_ud_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_ud is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_ud);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_ud", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_ud not exist\n", primary_key);
        }
        else
        {
            DE_INF("%s.lcd_power gpio_port=%d,gpio_port_num:%d, data:%d\n",primary_key, gpio_info->port, gpio_info->port_num, gpio_info->data);
            lcd_cfg->lcd_eink_ud_used= 1;
        }
    }

    //lcd_eink_rl
    lcd_cfg->lcd_eink_rl_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_rl_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_rl is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_rl);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_rl", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_rl not exist\n", primary_key);
        }
        else
        {
            DE_INF("%s.lcd_power gpio_port=%d,gpio_port_num:%d, data:%d\n",primary_key, gpio_info->port, gpio_info->port_num, gpio_info->data);
            lcd_cfg->lcd_eink_rl_used= 1;
        }
    }

    //lcd_eink_vgg
    lcd_cfg->lcd_eink_vgg_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_vgg_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_vgg is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_vgg);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_vgg", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_vgg not exist\n", primary_key);
        }
        else
        {
            DE_INF("%s.lcd_power gpio_port=%d,gpio_port_num:%d, data:%d\n",primary_key, gpio_info->port, gpio_info->port_num, gpio_info->data);
            lcd_cfg->lcd_eink_vgg_used= 1;
        }
    }

    //lcd_eink_vcom
    lcd_cfg->lcd_eink_vcom_used= 0;
    value = 1;
    ret = OSAL_Script_FetchParser_Data(primary_key, "lcd_eink_vcom_used", &value, 1);
    if(value == 0)
    {
        DE_INF("%s.lcd_eink_vcom is not used\n", primary_key);
    }
    else
    {
        gpio_info = &(lcd_cfg->lcd_eink_vcom);
        ret = OSAL_Script_FetchParser_Data(primary_key,"lcd_eink_vcom", (int *)gpio_info, sizeof(user_gpio_set_t)/sizeof(int));
        if(ret < 0)
        {
            DE_INF("%s.lcd_eink_vgg not exist\n", primary_key);
        }
...
//lcd_used
    ...
    //lcd_eink_config
    LCD_get_eink_config(primary_key, lcd_cfg);
...
__s32 pwm_set_duty_ns(__u32 channel, __u32 duty_ns)
{
	...
	/* For the new IA3328 chip
	 * Dirty hack to scale duty cycle to the minimum frontlight value
	 * Those are measured values for the lowest brightness:
	 * - duty_ns: 992 - active_cycle: 23808 (AP3019A - old chip)
	 * - duty_ns: 855 - active_cycle: 20520 (IA3328 - new chip)
	 */
	duty_ns = duty_ns * 855 / 992;
...
void LCD_eink_main_pwr_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_main_pwr_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_main_pwr), sizeof(user_gpio_set_t));
...
void LCD_eink_main_pwr_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_vneg_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_vneg_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_vneg), sizeof(user_gpio_set_t));
...
void LCD_eink_vneg_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_vpos_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_vpos_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_vpos), sizeof(user_gpio_set_t));
...
void LCD_eink_vpos_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_ud_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_ud_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_ud), sizeof(user_gpio_set_t));
...
void LCD_eink_ud_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_rl_on(__u32 sel)
{
	...
	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_rl_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_rl), sizeof(user_gpio_set_t));
...
void LCD_eink_rl_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_vgg_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_vgg_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_vgg), sizeof(user_gpio_set_t));
...
void LCD_eink_vgg_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
void LCD_eink_vcom_on(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 1;

	if (!gdisp.screen[sel].lcd_cfg.lcd_eink_vcom_used)
		return;

	memcpy(gpio_info, &(gdisp.screen[sel].lcd_cfg.lcd_eink_vcom), sizeof(user_gpio_set_t));
...
void LCD_eink_vcom_off(__u32 sel)
{
	user_gpio_set_t  gpio_info[1];
	__hdle hdl;
	__bool b_en = 0;
...
extern int encrypt_set_eink_vcom(__u32 sel);
...
__s32 Disp_lcdc_init(__u32 sel)
{
	...
	if (gdisp.screen[sel].lcd_cfg.lcd_used) {
		...
		if ((OSAL_sw_get_ic_ver() != 0xA) &&
			...
			pwm_info.duty_ns = pwm_info.duty_ns * 855 / 992;

			pwm_set_para(gpanel_info[sel].lcd_pwm_ch, &pwm_info);
		}
        else
        if(1 == gpanel_info[sel].lcd_pwm_not_used)  //We don't use pwm, 1-wire instead.
        {
            pwm_gpio = gpio_request_ex("lcd0_para", "lcd_pwm");
            gpio_get_one_pin_status(pwm_gpio, gpio_int_info, "lcd_pwm", 1);
            gpio_int_info[0].mul_sel=1; //Output

            hdl = OSAL_GPIO_Request(gpio_int_info, 1);
            OSAL_GPIO_Release(hdl, 2);

            gpio_get_one_pin_status(pwm_gpio, gpio_int_info, "lcd_pwm", 1);
            gpio_int_info[0].mul_sel=1; //Output
            gpio_set_one_pin_status(pwm_gpio,gpio_int_info,"lcd_pwm",1);
...
static void pb_gpio_set_pin(int value)
{
    ...
    gpio_write_one_pin_value(pwm_gpio, !!value, "lcd_pwm");
}
...
static int tps61161_set_bl(int level, int max, int min,bool boost)
{
    unsigned long flags;
...
static int scale(int value, int min_v, int max_v, int min_r, int max_r)
{
   int ret = value;
   ret -= min_v;
   ret = (ret * (max_r-min_r)) / (max_v - min_v);
   ret += min_r;
   return ret;
}
/*
...
__s32 BSP_disp_lcd_set_bright(__u32 sel, __u32 bright, __u32 from_iep)
{
    ...
    bool notboost;
    int tp_power_gpio=0;
	if (bright == 255 && isclosebacklight == false) 
    {
		/* Turning off frontlight */
		isclosebacklight = true;
		LCD_PWM_EN(0, 0); //close pwm module
        if(1 == gpanel_info[sel].lcd_pwm_not_used)  
        {
            tps61161_set_bl(0, TPS61161_MAX_BRIGHTNESS, 0,false);
        }
		LCD_BL_EN(0, 1);  //config lcd_bl_en pin to close lcd backlight
		return DIS_SUCCESS;

	}
    isclosebacklight = false;

	if ((OSAL_sw_get_ic_ver() != 0xA) &&
	   (gpanel_info[sel].lcd_pwm_not_used == 0) &&
	   (gdisp.screen[sel].lcd_cfg.lcd_used)) 
    {
        if (bright < 255)
        {
            /* Turning on frontlight */
            isclosebacklight = false;
            LCD_PWM_EN(0, 1); //open pwm module
            LCD_BL_EN(0, 0);  //config lcd_bl_en pin to open lcd backlight
        }

		if (gpanel_info[sel].lcd_pwm_pol == 0) 
        {
		  /* Try to make the curve look like exponential instead of linear */
		  if (bright > 180)
		  	duty_ns = 1000 - (( 255 - bright ) / 2 );
		  else if (bright  > 100)
			duty_ns = 1000 - ( 255 - bright ) + 30;
		  else if (bright > 50)
			duty_ns = 1000 - (( 255 - bright ) * 2 ) + 140;
		  else
			duty_ns = ( bright * 3 ) + 570;
		} 
        else 
        {
    ...
    else   //pwm not used, use 1-wire instead.
    {
        if(bright >=PWM_MAX_BRIGHTNESS/2)     //Without boost mode
        {
            newbright = scale((PWM_MAX_BRIGHTNESS - bright),0,PWM_MAX_BRIGHTNESS/2,1,TPS61161_MAX_BRIGHTNESS);
            notboost=true;
        }
        else
        {
            newbright = scale((PWM_MAX_BRIGHTNESS/2 - bright),0,PWM_MAX_BRIGHTNESS/2,10,TPS61161_MAX_BRIGHTNESS);
            notboost=false;
        }
        tps61161_set_bl(newbright, TPS61161_MAX_BRIGHTNESS, 0, notboost);
    }
