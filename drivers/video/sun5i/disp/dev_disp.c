...
void backlight_early_suspend(struct early_suspend *h)
{
	...
	while(Disp_eink_get_update_status(0)) {
		printk("backlight_early_suspend: msleep 100ms ... \n");
		msleep(100);
	}
...
int disp_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	while(Disp_eink_get_update_status(0)) {
		printk("disp_suspend: msleep 100ms ... \n");
		msleep(100);
	}
...
void disp_shutdown(struct platform_device *pdev)
{
	...
	while(Disp_eink_get_update_status(0)) {
		printk("disp_shutdown: msleep 100ms ... \n");
		msleep(100);
	}
...
long disp_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	coordinate[0] = (*(unsigned long*)(karg+4));
	coordinate[1] = (*(unsigned long*)(karg+5));
	coordinate[2] = (*(unsigned long*)(karg+6));
	coordinate[3] = (*(unsigned long*)(karg+7));
        ...
        case DISP_CMD_EINK_UPDATE:
	{
		...
		ret = Disp_eink_update(ubuffer[0], ubuffer[1], ubuffer[2], coordinate);
...
__u32 Drv_disp_get_fb_bpp(__u32 fb_id)
{
    return g_fbi.fbinfo[fb_id]->var.bits_per_pixel;
}
...
{    
    return BSP_disp_layer_request(sel, mode);   //\C9\EA\C7\EBӲ\BC\FEͼ\B2\E3
}

__s32 Drv_disp_layer_release(__u32 sel, __hdle hlyr)
{    
    return BSP_disp_layer_release(sel, hlyr);   //\CAͷ\C5Ӳ\BC\FEͼ\B2\E3
}
