...
#include "dev_disp.h"
#include "de_bsp/de/disp_eink_data.h"
...
__s32 Display_Fb_Request(__u32 fb_id, __disp_fb_create_para_t *fb_para)
{
	...
	__wrn("Display_Fb_Request,fb_id:%d\n", fb_id);
    __wrn("fb_para->buffer_num: %d \n", fb_para->buffer_num);

__wrn("Get width : %d  get height : %d \n", BSP_disp_get_screen_width(0),BSP_disp_get_screen_height(0));
    ...
    __wrn("info->fix.smem_len: %d info->var.xres : %d info->var.yres : %d", info->fix.smem_len,info->var.xres,info->var.yres);
...
__wrn("Get width : %d  get height : %d \n", BSP_disp_get_screen_width(0),BSP_disp_get_screen_height(0));
        for(i=0; i<8; i++)
        {
            ...
            g_fbi.fbinfo[i]->var.xres            = ((BSP_disp_get_screen_width(0) -EINK_HYNC)*4); //EINK_PANEL_W;
            g_fbi.fbinfo[i]->var.yres            = BSP_disp_get_screen_height(0) - EINK_VYNC;  //EINK_PANEL_H;
            g_fbi.fbinfo[i]->var.xres_virtual    = ((BSP_disp_get_screen_width(0) -EINK_HYNC)*4);
            g_fbi.fbinfo[i]->var.yres_virtual    = (BSP_disp_get_screen_height(0) - EINK_VYNC)*2;
    ...
    if(g_fbi.disp_init.b_init)
    {
        ...
        for(i = 0; i<fb_num; i++)
        {
            ...
            */
            //modified by heyihang
            fb_para.width = ((BSP_disp_get_screen_width(0) -EINK_HYNC)*4);
            fb_para.height = BSP_disp_get_screen_height(0) - EINK_VYNC;
            fb_para.output_width = ((BSP_disp_get_screen_width(0) -EINK_HYNC)*4);
            fb_para.output_height = BSP_disp_get_screen_height(0) - EINK_VYNC;
            ...
            __wrn("Launching Display_Fb_Request, i : %d\n", i);
