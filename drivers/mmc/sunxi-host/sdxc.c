...
static void sdxc_send_cmd(struct sunxi_mmc_host* smc_host, struct mmc_command* cmd)
{
	...
	{
		if ((cmd->flags & MMC_CMD_MASK) != MMC_CMD_BC)
		{
			cmd_val |= SDXC_RspExp;
...
//			SMC_INFO("sg %d, frag %d, remain %d, des[%d](%08x): "
...
static int sdxc_prepare_dma(struct sunxi_mmc_host* smc_host, struct mmc_data* data)
{
	...
	writel(smc_host->sg_dma, SDXC_REG_DLBA);
...
//		smc_host->pdev->id, raw_int, smc_host->int_sum, msk_int, idma_inte, idma_int);
	...
	else if (smc_host->wait == SDC_WAIT_CMD_DONE
		&& (msk_int&SDXC_CmdDone)
...
_out_:
	if (req->data)
	{
		...
		if (!(data->flags & MMC_DATA_WRITE) && (readl(SDXC_REG_STAS) & SDXC_DataFSMBusy))
		{
			if ((readl(SDXC_REG_STAS) & SDXC_DataFSMBusy)
				...
				&& (readl(SDXC_REG_STAS) & SDXC_DataFSMBusy))
					SMC_DBG("mmc %d fsm busy 0x%x len %d\n",
						smc_host->pdev->id, readl(SDXC_REG_STAS),
		...
		{
			int i = 0;
			char* dirstr = data->flags & MMC_DATA_WRITE ? "== Tx Data =="
									: "== Rx Data ==";
			u32 sg_len = data->sg_len;

			SMC_MSG("%s\n", dirstr);
			for (i=0; i<sg_len; i++) {
				char sgstr[8] = {0};
				sprintf(sgstr, "sg[%d]", i);
				hexdump(sgstr, sg_virt(&data->sg[i]),
					data->sg[i].length > 64 ? 64 : data->sg[i].length);
			}
		}
		#endif
	}
