...
static int sunximmc_set_src_clk(struct sunxi_mmc_host* smc_host)
{
	...
	SMC_INFO("smc %d, source = %s, src_clk = %u, mclk %u, \n", smc_host->pdev->id,
		name[smc_host->clk_source], (unsigned)clk_get_rate(source_clock), smc_host->mod_clk);
...
static int sunximmc_resource_request(struct sunxi_mmc_host *smc_host)
{
	...
	/* smc address remap */
	smc_host->smc_base_res = request_mem_region(smc_host->smc_base_res->start,
						RESSIZE(smc_host->smc_base_res), pdev->name);
	if (!smc_host->smc_base_res)
	{
	...
	smc_host->smc_base = ioremap(smc_host->smc_base_res->start,
					RESSIZE(smc_host->smc_base_res));
...
static struct resource sunximmc_resources[SUNXI_MMC_HOST_NUM][2] = {
	...
	{/* mmc0 */
		{ .start = SMC_BASE(0),     .end = SMC_BASE(0)+0x1000-1, .flags	= IORESOURCE_MEM},
		{ .start = INTC_IRQNO_SMC0, .end = INTC_IRQNO_SMC0,      .flags	= IORESOURCE_IRQ},
	},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC1_USED)
	{/* mmc1 */
		{ .start = SMC_BASE(1),     .end = SMC_BASE(1)+0x1000-1, .flags	= IORESOURCE_MEM},
		{ .start = INTC_IRQNO_SMC1, .end = INTC_IRQNO_SMC1,      .flags	= IORESOURCE_IRQ},
	},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC2_USED)
	{/* mmc2 */
		{ .start = SMC_BASE(2),     .end = SMC_BASE(2)+0x1000-1, .flags	= IORESOURCE_MEM},
		{ .start = INTC_IRQNO_SMC2, .end = INTC_IRQNO_SMC2,      .flags	= IORESOURCE_IRQ},
	},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC3_USED)
	{/* mmc3 */
		{ .start = SMC_BASE(3),     .end = SMC_BASE(3)+0x1000-1, .flags	= IORESOURCE_MEM},
		{ .start = INTC_IRQNO_SMC3, .end = INTC_IRQNO_SMC3,      .flags	= IORESOURCE_IRQ},
	},
	#endif
};
...
static struct platform_device awmmc_device[SUNXI_MMC_HOST_NUM] = {
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC0_USED)
	[0] = {.name = DRIVER_NAME, .id = 0, .num_resources = ARRAY_SIZE(sunximmc_resources[0]),
		.resource = &sunximmc_resources[0][0], .dev = {}},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC1_USED)
	[1] = {.name = DRIVER_NAME, .id = 1, .num_resources = ARRAY_SIZE(sunximmc_resources[1]),
		.resource = &sunximmc_resources[1][0], .dev = {}},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC2_USED)
	[2] = {.name = DRIVER_NAME, .id = 2, .num_resources = ARRAY_SIZE(sunximmc_resources[2]),
		.resource = &sunximmc_resources[2][0], .dev = {}},
	#endif
	#if (SUNXI_MMC_USED_CTRL & SUNXI_MMC3_USED)
	[3] = {.name = DRIVER_NAME, .id = 3, .num_resources = ARRAY_SIZE(sunximmc_resources[3]),
		.resource = &sunximmc_resources[3][0], .dev = {}},
	#endif
};
