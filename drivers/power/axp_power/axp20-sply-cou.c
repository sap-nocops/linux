...
static uint8_t magnet_state = 0;	//0 = No magnet on. 1 = magnet on.
static uint8_t button_state = 0;	//0 = button not pressed. 1 = button pressed.
static uint8_t button_loop_state = 0;	//change every time it is pressed. 1->0->1 etc...
static uint8_t deep_state = 0;	//if = 2, then we are in deep s...leep
...
static int pmu_hall_sensor_value = 0;
#ifdef CONFIG_HAS_EARLYSUSPEND
...
static uint8_t read_reg_val;
static int change_flag = 0;

int gpio_hall_sensor = 0;
static user_gpio_set_t  gpio_hall_sensor_info[1];
static uint8_t is_hall_sensor = 0;
...
static void axp_set_startup_sequence(struct axp_charger *charger)
{
	...
	//axp_write(charger->master,0x03,0xC2);  //set EXTEN power up at step 2 and 32ms step by step
	axp_write(charger->master,0x03,0x86);  //Jack set EXTEN power up at step 4 and 16ms step by step
	...
	axp_write(charger->master,0x04,0x10); //Jack set Core-VDD power up at step 2
...
static void axp_change(struct axp_charger *charger)
{
	...
	DBG_APP_MSG("charger->usb_valid = %d\n",charger->usb_valid);
	if(!charger->usb_valid){
		DBG_APP_MSG("set usb vol-lim to %d mV, cur-lim to %d mA\n",pmu_usbvolnew,pmu_usbcurnew);
		...
		if(pmu_usbcurnew){
			...
			if(var >= 900000)
				...
				DBG_APP_MSG("set usb limit current error,%d mA\n",pmu_usbcur);	
		}
		else
		...
		if(pmu_usbvolnew){
			...
			if(var >= 4000000 && var <=4700000){
				...
				DBG_APP_MSG("set usb limit voltage error,%d mV\n",pmu_usbvol);	
		}
		else
	...
	sysfs_notify(&charger->usb.dev->kobj, NULL, "online");
}
...
//	w[8] = (uint8_t) (((uint64_t) event >> 32) & 0xFF);
	...
	gpio_hall_read_value = gpio_read_one_pin_value(gpio_hall_sensor, "pmu_hall_sensor");
	DBG_APP_MSG("---------Hall Sensor Value : %d .\n",gpio_hall_read_value);

	axp_read(charger->master,POWER20_INTEN5, &val);
	val |= 0x08;
	axp_write(charger->master,POWER20_INTEN5, val);
	val = 0x86;


	axp_read(charger->master,POWER20_GPIO3_CTL, &val);	//fetch default value.
	DBG_APP_MSG("---------value before : 0x%x\n",val);

	if ( (0 == magnet_state) && (1 == gpio_hall_read_value) && (0 == button_state) )
	{
		val = 0x46;
	}

	if ( (0 == magnet_state) && (1 == gpio_hall_read_value) && (1 == button_state) )
	{
		val = 0x86;
	//	button_state=0;
	}	

	if ( (1 == magnet_state) && (1 == gpio_hall_read_value) && (1 == button_state) )
	{
		val = 0x46;
		button_state=0;
		magnet_state=0;
	...
	if ( (1 == magnet_state) && (0 == gpio_hall_read_value) && (0 == button_state) )
	{
		val = 0x86;
	}

	if ( (1 == magnet_state) && (1 == gpio_hall_read_value) && (0 == button_state) )
	{
		val = 0x46;
	//	deep_state=1;
		magnet_state=0;
	...
	if ( (1 == magnet_state) && (0 == gpio_hall_read_value) && (1 == button_state) )
	{
		val = 0x46;
		button_state=0;
		magnet_state=0;
	...
	if ( (0 == magnet_state) && (0 == gpio_hall_read_value) && (1 == button_state) )
	{
		DBG_APP_MSG("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!NINDBLOZ 0 0 1 \n\n");
		val = 0x86;
	}	

	if ( (0 == magnet_state) && (0 == gpio_hall_read_value) && (0 == button_state) )
	{
		DBG_APP_MSG("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!NINDBLOZ2 0 0 0 \n\n");
		val = 0x46;	//Ou 86 ?
	}

	axp_write(charger->master,POWER20_GPIO3_CTL, val);


	DBG_APP_MSG("---------val : 0x%x magnet_state : %d gpio_hall_read_value : %d button_state : %d  deep_state : %d!\n",val, magnet_state,  gpio_hall_read_value, button_state,deep_state);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(var >= 4000000 && var <=4700000){
		tmp = (var - 4000000)/100000;
		//DBG_APP_MSG("tmp = 0x%x\n",tmp);
		axp_read(charger->master, AXP20_CHARGE_VBUS,&val);
		val &= 0xC7;
		val |= tmp << 3;
		//DBG_APP_MSG("val = 0x%x\n",val);
		axp_write(charger->master, AXP20_CHARGE_VBUS,val);
	}
	return count;
}
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	if(	flag_notfristin	){
		...
		*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±ÆäÐ¡ÓÚ95£¬ocv°Ù·Ö±È´óÓÚ95£¬ÇÒÔÚ³äµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿×Ô¼Ó*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±ÆäÐ¡ÓÚ100£¬ocv´óÓÚ4.1V£¬Íâ²¿³äµçÆ÷ÔÚ£¬ÇÒÎ´ÔÚ³äµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿×Ô¼Ó£¬Ö±µ½100*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±ÆäÐ¡ÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö+5£©£¬ocv°Ù·Ö±È´óÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö±È+4£©£¬ÇÒÔÚ·ÅµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿±£³Ö²»±ä£¬µÈocv°Ù·Ö±È½µÏÂÀ´*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±Æä´óÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö+12£©£¬ocv°Ù·Ö±ÈÐ¡ÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö±È+13£©£¬ÇÒÔÚ·ÅµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿×Ô¼õ*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±Æä´óÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö+4£©£¬ocv°Ù·Ö±ÈÐ¡ÓÚ£¨µç³Ø×ÜÈÝÁ¿Ð£Õý°Ù·Ö±È+5£©£¬ÇÒÔÚ·ÅµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿×Ô¼õ*/
		...
		/*µç³ØÊ£ÓàÈÝÁ¿Ð£Õý£¬µ±Æä´óÓÚµÈÓÚ100£¬ÇÒ»¹ÔÚ³äµçÊ±£¬µç³ØÊ£ÓàÈÝÁ¿°Ù·Ö±ÈÉèÖÃÎª99*/
	...
	if(charger->rest_vol - pre_rest_vol){
		DBG_APP_MSG("battery vol change: %d->%d \n", pre_rest_vol, charger->rest_vol);
		pre_rest_vol = charger->rest_vol;
...
static int axp_battery_probe(struct platform_device *pdev)
{
	...
	if(ret) {
		DBG_APP_MSG("Unable to Register the power key\n");
	}
	...
	if (pdata->chgcur > 1800000 ||
		...
		DBG_APP_MSG("charger milliamp is too high or target voltage is over range\n");
		return -EINVAL;
	}

	if (pdata->chgpretime < 40 || pdata->chgpretime >70 ||
		pdata->chgcsttime < 360 || pdata->chgcsttime > 720){
		DBG_APP_MSG("prechaging time or constant current charging time is over range\n");
		return -EINVAL;
	}
	...
	if((pmu_usbcur) && (pmu_usbcur_limit)) {
		...
		if(var >= 900000) {
			...
			DBG_APP_MSG("set usb limit current error,%d mV\n",pmu_usbcur);	
		}
  	} else {
		axp_set_bits(charger->master, AXP20_CHARGE_VBUS, 0x03);
	}
	...
	if (var) {
		DBG_APP_MSG("[AXP]axp driver uning configuration failed(%d)\n", __LINE__);
		pmu_suspendpwroff_vol = 3500;
		DBG_APP_MSG("[AXP]pmu_suspendpwroff_vol = %d\n",pmu_suspendpwroff_vol);
	}
	...
	/* open/close set */
	DBG_APP_MSG("pmu_pekoff_time = %d\n",pmu_pekoff_time);
	DBG_APP_MSG("pmu_pekoff_en = %d\n",pmu_pekoff_en);
	DBG_APP_MSG("pmu_peklong_time = %d\n",pmu_peklong_time);
	DBG_APP_MSG("pmu_pekon_time = %d\n",pmu_pekon_time);
	DBG_APP_MSG("pmu_pwrok_time = %d\n",pmu_pwrok_time);
	DBG_APP_MSG("pmu_pwrnoe_time = %d\n",pmu_pwrnoe_time);
	DBG_APP_MSG("pmu_intotp_en = %d\n",pmu_intotp_en);
	...
	DBG_APP_MSG("POWER20_PEK_SET:%d-->0x%x\n",__LINE__,val);
	...
	DBG_APP_MSG("POWER20_HOTOVER_CTL:%d-->0x%x\n",__LINE__,val);
	...
	if(0 == bat_cap)
		bat_cap = 1;	//To avoid divide by 0 error
	...
	if (var) {
		DBG_APP_MSG("axp driver uning configuration failed(%d)\n", __LINE__);
	...
	if (var) {
		DBG_APP_MSG("axp driver uning configuration failed-pmu_usbvol_pc\n");
		pmu_usbvolnew = 4000;
		DBG_APP_MSG("pmu_usbvolnew = %d\n",pmu_usbvolnew);
	} else {
	...
	if (var) {
		DBG_APP_MSG("axp driver uning configuration failed-pmu_usbcurnew\n");
		pmu_usbcurnew = 500;
		DBG_APP_MSG("pmu_usbcurnew = %d\n",pmu_usbcurnew);
	} else {
...
DBG_APP_MSG("----------------Dans axp_battery_probe,lancement config.\n");
	axp_read(charger->master,POWER20_INTEN5, &val);
	val |= 0x08;
	DBG_APP_MSG("Valeur : %d.\n",val);
	axp_write(charger->master,POWER20_INTEN5, val);


//	axp_set_bits(charger->master,POWER20_GPIO3_CTL,0x44);
	...
	gpio_hall_sensor = gpio_request_ex("pmu_para", "pmu_hall_sensor");
	if (!gpio_hall_sensor ) 
	{
		DBG_APP_MSG("Cannot fetch hall sensor pin.\n");
		is_hall_sensor = 0;
	} 
	else 
	{	
		gpio_get_one_pin_status(gpio_hall_sensor, gpio_hall_sensor_info, "pmu_hall_sensor", 1);
		DBG_APP_MSG("==============================%s, %d: gpio_hall_sensor_info, port = %d, port_num = %d. \n", __func__, __LINE__, \
			gpio_hall_sensor_info[0].port, gpio_hall_sensor_info[0].port_num);

		gpio_hall_read_value = gpio_read_one_pin_value(gpio_hall_sensor, "pmu_hall_sensor");

		if(1 == gpio_hall_read_value)	//No magnet on
			val = 0x46;
		else
			val = 0x86;
	...
	axp_read(charger->master,POWER20_LDO3OUT_VOL,&read_reg_val);
#ifdef DEBUG_LDO3
	DBG_APP_MSG("axp209 : ldo3 debug reg value at init %u \n",read_reg_val);
#endif
	return ret;
...
static int axp20_suspend(struct platform_device *dev, pm_message_t state)
{
	...
	DBG_APP_MSG("pmu_suspend_chgcur = %d\n", pmu_suspend_chgcur);
#endif
	/* timer */
...
#ifdef DEBUG_LDO3
	uint8_t value_temp;
	axp_read(charger->master, POWER20_LDO234_DC23_CTL, &value_temp);
	DBG_APP_MSG("axp avant suspend valeur reg 12H : %u \n",value_temp);
#endif
	axp_write(charger->master, POWER20_LDO234_DC23_CTL, LDO3_DIS);
...
*/
	...
	gpio_hall_read_value = gpio_read_one_pin_value(gpio_hall_sensor, "pmu_hall_sensor");
	axp_read(charger->master,POWER20_GPIO3_CTL, &val);
	DBG_APP_MSG("Dans suspend : val : 0x%x magnet_state : %d gpio_hall_read_value : %d button_state : %d  deep_state : %d!\n",val, magnet_state,  gpio_hall_read_value, button_state,deep_state);
	...
	if ( (0 == magnet_state) && (0 == button_state) )
		deep_state++;
	else
		deep_state=1;

/*	if ( (0 == magnet_state) && (1 == gpio_hall_read_value) && (0 == button_state) && (2 == deep_state))
	{
		val = 0x86;
	}
*/
	if (3 == deep_state)	//We are in deep state. Now, we need to change the configuration.
	{
		DBG_APP_MSG("*************************deep state detected.\n");
		if ( (0 == magnet_state) && (1 == gpio_hall_read_value) && (0 == button_state) )
		{
			val = 0x86;
		}
	...
	axp_read(charger->master,POWER20_GPIO3_CTL, &rising_value);
	DBG_APP_MSG("Fin suspend : val : 0x%x magnet_state : %d gpio_hall_read_value : %d button_state : %d  deep_state : %d!\n",val, magnet_state,  gpio_hall_read_value, button_state,deep_state);
...
static int axp20_resume(struct platform_device *dev)
{
	...
	int	Cou_Correction_Flag;
	int bat_valtage;
	//*****Ð£Õýµç³ØÈÝÁ¿ÅäÖÃÆ«´óµ¼ÖÂµçÁ¿ÏÔÊ¾²»×¼*******//
	...
	if(v[0] &= 0x04){
		...
		if(bat_valtage > 4080){
			...
			if(Cou_Correction_Flag){
				DBG_APP_MSG("[AXP20-MFD] ----------charger finish need to be corrected-----------\n");
				DBG_APP_MSG("[AXP20-MFD] ----------correct the coulomb counter-----------\n");
	...
	uint8_t value_temp;
	axp_read(charger->master, POWER20_LDO234_DC23_CTL, &value_temp);
	DBG_APP_MSG("axp debut resume valeur reg 12H : %u \n",value_temp);
#endif
	//set LDO3 back to on
	axp_write(charger->master, POWER20_LDO234_DC23_CTL, LDO3_EN);
	//*******************************end**************************************//
...
/******************************³ö´í´¦Àí£¬Èç¹û³õÊ¼Pre_rest_cap¹À¼ÆÆ«´ó£¬µ¼ÖÂ³äµçÊ±ÓÃ¿âÂØ¼Æ¼ÆËãµÄµçÁ¿´óÓÚ100%**************************/
		if(charger->rest_vol > 100) {
			charger->rest_vol =	100;
		} else if(charger->rest_vol < 0) {
			charger->rest_vol =	0;
...
/******************************³ö´í´¦Àí£¬Èç¹û³õÊ¼Pre_rest_cap¹À¼ÆÆ«Ð¡£¬µ¼ÖÂ³äÂúºóÓÃ¿âÂØ¼Æ¼ÆËãµÄµçÁ¿µÍÓÚ100%****************/
...
/*****************************************³ö´í´¦Àí£¬Èç¹û»¹ÔÚ³äµç£¬¼´Ê¹·µ»Ø100%£¬Ò²ÖÃÎª99%**********************************/
		if((charger->bat_current_direction == 1) &&	(charger->rest_vol >= 100)) {
			DBG_APP_MSG("Resume: charging->charger->rest_vol = %d ->99\n",charger->rest_vol);
			charger->rest_vol =	99;
			Cur_CoulombCounter = Get_Bat_Coulomb_Count(charger);
			saved_cap =	99 - 100 * (Cur_CoulombCounter)	/ bat_cap;
			Set_Rest_Cap(charger,saved_cap);
...
if(axp_debug){
	...
	charger->disibat = 0;
	schedule_delayed_work(&charger->work, charger->interval);
	axp_write(charger->master,POWER20_LDO3OUT_VOL,read_reg_val);
#ifdef DEBUG_LDO3
	axp_read(charger->master, POWER20_LDO234_DC23_CTL, &value_temp);
	DBG_APP_MSG("axp fin resume valeur reg 12H : %u \n",value_temp);
#endif
	return 0;
}
...
static void axp20_shutdown(struct platform_device *dev)
{
	...
	if(pmu_shutdown_chgcur == 0)
		...
		DBG_APP_MSG("pmu_shutdown_chgcur = %d\n", pmu_shutdown_chgcur);
