...
#define SUSPEND_WAKEUP_SRC_PIO      (1<<6)  /* set time to pio  */

extern unsigned int standby_wakeup_event;
...
#define STANDBY_PIO_BUF_SIZE    64
...
struct aw_standby_para{
    ...
    unsigned char  pio_buffer[STANDBY_PIO_BUF_SIZE];
    unsigned int   pio_data_cnt;
};
